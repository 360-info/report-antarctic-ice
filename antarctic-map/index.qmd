...
Plot.plot({
  ...
  marks: [
    ...
    storyStep >= 3 ? Plot.geo(seaIceOutline, {
      // fill: "url(#stripes)",
      stroke: "#333333",
      opacity: d => {
        let dNum = parseInt(d.properties.month)
        return dNum => (dNum + 12 - currentMonth) % 12
      },
      strokeWidth: 1,
      // fillOpacity: 0.75,
      render: fadeLayerIn
...
monthTimer = {
  ...
  yield i
  while(true) {
    yield Promises.tick(1000).then(() => i++ % 12)
  }
}
// only run the timer when the sea ice layer is visible to avoid
// other layers flickering
