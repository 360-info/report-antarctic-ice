...
antarcticMap = Plot.plot({
  ...
  marks: [
    ...
    /* there's some complexity here with duplicate layers as the render
       transform is a bit tough to work with and i don't want layers
       blinking on and off twice a second */
    storyStep == 1 || storyStep == 2 ? landIceOutlineLayerWithFade : null,
    storyStep == 2 ? iceShelfArrowLayerWithFade : null,
    storyStep == 2 ? iceShelfLabelLayerWithFade : null,
    storyStep == 3 ? seaIceRegionLayer : null,
    storyStep == 3 ? landIceOutlineLayer : null,
    storyStep == 3 ? seaIceLabelLayer : null,
  ],
  height: 800,
  insetTop: 0
})
...
fadeLayerIn = (i, s, v, d, c, next) => {
  ...
  console.log("---- RUNNING fadeLayerIn ----")
...
monthTimer = {
  ...
  yield i
  while(storyStep >= 3) {
    yield Promises.tick(300).then(() => i++ % 12)
  }
}
```
